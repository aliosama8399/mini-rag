...
async def _index_data_content(task_instance, project_id: int, do_reset: int):
    ...
    try:
        ...
        logger.info("Setup utils loaded in data_indexing task")
        ...
        logger.info(
            f"Vector collection ensured | name={collection_name} do_reset={do_reset}"
        )
        ...
        while has_records:
            ...
            logger.info(
                f"Indexed batch | size={len(page_chunks)} total_indexed={inserted_items_count}"
            )
        ...
        logger.error(f"Task failed in data_indexing: {str(e)}")
        raise
    finally:
        try:
            if db_engine:
            ...
            logger.error(f"Task failed while cleaning in data_indexing: {str(e)}")